...
export function CodBarra() {
  ...
  useEffect(() => {
    const codigoEjemplo = '7791290794078';
  
    // Hago la consulta al backend que ya tengo publicado
    fetch(`http://186.123.181.61:3000/buscar/${codigoEjemplo}`)
  ...
  return (
    <View style={styles.containerPrincipal}>
      <View style={styles.container}>
        ...
        {/* Listado de resultados que vienen del backend */}
        <FlatList
          data={results}
          keyExtractor={item => item.codigoBarra || item.id || Math.random().toString()}
          renderItem={({ item }) => (
            <View style={styles.resultItem}>
              <Text style={styles.resultText}>
                {item.descripcion} - ${item.preciosLista?.precio1 || 'N/A'}
              </Text>
            </View>
          )}
        />
